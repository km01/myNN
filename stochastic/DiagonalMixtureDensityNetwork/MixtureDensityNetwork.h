...
public: 
		...
		for (int k = 0; k < n_group; k++) {
			...
			if(cum >= u) {
				for (int d = 0; d < n_dim; d++) {
					y[d] = km::STD(rEngine) * stddev[k][d] + mean[k][d];
				}
				break;
			}
		}
	...
	double fit(Darr& x, Darr& y) {
		...
		for (int k = 0; k < n_group; k++) {
			nn_out_grad[k] = mixing_coeff[k] - (mixing_coeff[k] * conditional[k] / likelihood_of_y);
			for (int i = 0; i < n_dim; i++) {
				nn_out_grad[n_group + k * n_dim + i] =  (conditional[k] * mixing_coeff[k] 
					* (mean[k][i] - y[i]))/ (stddev[k][i] * stddev[k][i] * likelihood_of_y);
				nn_out_grad[n_group + n_group * n_dim + k * n_dim + i] 
					= (1.0 - ((mean[k][i] - y[i]) / stddev[k][i]) * ((mean[k][i] - y[i]) / stddev[k][i])) * 
					((conditional[k] * mixing_coeff[k])/( likelihood_of_y));
			}
		}
...
class DiagCovMDN2 : public Sequential {
public:
	...
	void sampling(Darr& x, Darr& y) {
		...
		for (int k = 0; k < n_group; k++) {
			...
			for (int i = 0; i < n_dim; i++) {
				mean[k][i] = nn_out[n_group + k * n_dim + i];
				if (nn_out[n_group + n_group * n_dim + k * n_dim + i] > 0) {
					stddev[k][i] = nn_out[n_group + n_group * n_dim + k * n_dim + i] + 1.0;
				}
				else {
					stddev[k][i] = exp(nn_out[n_group + n_group * n_dim + k * n_dim + i]);
				}
		...
		for (int k = 0; k < n_group; k++) {
			...
			for (int i = 0; i < n_dim; i++) {
				if (nn_out[n_group + n_group * n_dim + k * n_dim + i] > 0) {
					nn_out_grad[n_group + n_group * n_dim + k * n_dim + i]
						= (1.0 - ((mean[k][i] - y[i]) / stddev[k][i]) * ((mean[k][i] - y[i]) / stddev[k][i])) *
						((conditional[k] * mixing_coeff[k]) / (likelihood_of_y * stddev[k][i]));
				}
				else {
					nn_out_grad[n_group + n_group * n_dim + k * n_dim + i]
						= (1.0 - ((mean[k][i] - y[i]) / stddev[k][i]) * ((mean[k][i] - y[i]) / stddev[k][i])) *
