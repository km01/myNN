...
public:
	...
	int n_bundle;
	double learning_rate;
	followeroptimizer(nn* follower, nn* followee) {
		optim_alloced = false;
		collect(follower, followee);
	}
	...
	void collect(nn* _follower, nn* _followee) {
		if (optim_alloced) {
			delete[] params;
			delete[] grad;
			delete[] n_params;
		...
		vector<double*> trash;
		vector<int> len_bag;
		_follower->delegate(p_bag, trash, len_bag);
		n_bundle = p_bag.size();
		params = new double*[n_bundle];
		grad = new double*[n_bundle];
		n_params = new int[n_bundle];
		...
		p_bag.clear();
		len_bag.clear();
		_followee->delegate(p_bag, trash, len_bag);
		for (int n = 0; n < n_bundle; n++) {
			assert(n_params[n] == len_bag[n]);
			grad[n] = p_bag[n];
		}
		optim_alloced = true;
	}
