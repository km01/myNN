...
public:
	...
	virtual void save(ofstream& fout) {
		...
		fout << in_dim.h << endl;
		fout << in_dim.w << endl;
		fout << kernel_dim.c << endl;
		fout << kernel_dim.h << endl;
		fout << kernel_dim.w << endl;
		fout << out_dim.c << endl;
		fout << out_dim.h << endl;
		fout << out_dim.w << endl;
		fout << h_strd << endl;
	...
	virtual void load(ifstream& fin) {
		...
		getline(fin, buffer);
		int _hstrd = atoi(buffer.c_str());
		getline(fin, buffer);
		int _wstrd = atoi(buffer.c_str());
...
public:
	...
	virtual void load(ifstream& fin) {
		string buffer;
		getline(fin, buffer);
		int _n_group = atoi(buffer.c_str());
		getline(fin, buffer);
		int _n_elems = atoi(buffer.c_str());
		create(_n_group, _n_elems);
		for (int i = 0; i < n_group; i++) {
			getline(fin, buffer);
			scale[i] = atof(buffer.c_str());
			getline(fin, buffer);
			shift[i] = atof(buffer.c_str());
			getline(fin, buffer);
