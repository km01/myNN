...
public:
	...
	dqn() { is_allocated = false; }
	dqn(enviroment& _env, double _gamma, double _e) {
		env = &_env;
		n_state = env->n_state;
	...
	~dqn() {
		if (is_allocated) {
			...
			km_2d::free(batch_x, net->batch_size);
			km_2d::free(batch_y, net->batch_size);
			km_2d::free(dLoss, net->batch_size);
	...
	double fit() {
		...
		for (int i = 0; i < nb_batch; i++) {
			...
			loss += loss_fn.batch_loss_prime(dLoss, net->batch_output, batch_y, net->batch_size, n_action);
