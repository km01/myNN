...
public:
	...
	virtual void ddqn_selecter_update(unit* evaluater, const double& learing_rate) = 0;
...
public:
	...
	unit* clone() {
		perceptrons* clone = new perceptrons(input_size, output_size, activator->formula,
			use_bias);
		for (int i = 0; i < weight_len; i++) {
			clone->weight[i] = weight[i];
		}
		if (use_bias) {
	...
	virtual void ddqn_selecter_update(unit* evaluater, const double& learing_rate) {
		perceptrons* eval = (perceptrons*)evaluater;
		for (int i = 0; i < weight_len; i++) {
			weight[i] = weight[i] * (1.0 - learing_rate) + eval->weight[i] * (learing_rate);
		}
		if (use_bias) {
			for (int i = 0; i < bias_len; i++) {
				bias[i] = bias[i] * (1.0 - learing_rate) + eval->bias[i] * (learing_rate);
			}
		}
	}
