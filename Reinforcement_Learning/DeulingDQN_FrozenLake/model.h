...
public:
	...
	bool allocated_single_memory;
	nn(){}

	nn(int n) {
		allocated_batch_memory = false;
		allocated_single_memory = false;
		n_layer = n;
		batch_size = -1;
		end_layer = n_layer - 1;
	...
	nn(number_of_units num) : nn(num.nb_units) {}
	...
	void alloc_single_memory() {
		if (!allocated_single_memory) {
			input_size = layer[0]->input_size;
			output_size = layer[end_layer]->output_size;
			single_output = new double[output_size];
			allocated_single_memory = true;
		}
		allocated_single_memory = true;
	}
	...
	void single_feed_forward(double* const& single_input) {
		...
		for (int i = 0; i < n_layer - 1; i++) {
			layer[i]->single_forward_prop(layer[i + 1]->single_input_container);
		}
		layer[end_layer]->single_forward_prop(single_output);
		km_1d::argmax(single_argmax,single_output, output_size);
	}
