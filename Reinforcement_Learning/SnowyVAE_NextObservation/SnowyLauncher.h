...
public:
	...
	SnowyVisualizer(snowy::Snowy* _snowy) {
		...
		recon_ob = new double[observation_size];
	...
	void drawWorld() {
		...
		for (int h = 0; h < snowy->height; h++) {
			for (int w = 0; w < snowy->width; w++) {
				...
				color_r = (recon_ob[h * snowy->width + w]) * void_r
					+ (recon_ob[n_cells + h * snowy->width + w]) * snow_r
					+ (recon_ob[n_cells + n_cells + h * snowy->width + w]) * man_r;
				color_g = (recon_ob[h * snowy->width + w]) * void_g
					+ (recon_ob[n_cells + h * snowy->width + w]) * snow_g
					+ (recon_ob[n_cells + n_cells + h * snowy->width + w]) * man_g;
				color_b = (recon_ob[h * snowy->width + w]) * void_b
					+ (recon_ob[n_cells + h * snowy->width + w]) * snow_b
					+ (recon_ob[n_cells + n_cells + h * snowy->width + w]) * man_b;

				glColor3d(color_r, color_g, color_b);
				glPushMatrix(); glBegin(GL_POLYGON); {
					glVertex2f(fr_x + cell_size * w, fr_y - cell_size * (snowy->height - 1 - h));
					glVertex2f(fr_x + cell_size * (w + 1), fr_y - cell_size * (snowy->height - 1 - h));
					glVertex2f(fr_x + cell_size * (w + 1), fr_y - cell_size * (snowy->height - h));
					glVertex2f(fr_x + cell_size * w, fr_y - cell_size * (snowy->height - h));
				} glEnd(); glPopMatrix();
			}
		}
	}
	...
	void drawTextInfo() {
		...
		if (is_terminal) {
			...
			string obRecon = "E[O|S]:";
			drawBitmapText((char*)obRecon.c_str(), fr_x -0.12, fr_y - 0.03f, 0.f);

			string obL = "E[O'|O,  A=LEFT]:";
			drawBitmapText((char*)obL.c_str(), f0_x + margin, f0_y - 0.03f, 0.f);

			string obR = "E[O'|O,A=RIGHT]:";
			drawBitmapText((char*)obR.c_str(), f1_x + margin, f1_y - 0.03f, 0.f);

			string obH = "E[O'|O, A=HOLD]:";
			drawBitmapText((char*)obH.c_str(), f2_x + margin, f2_y - 0.03f, 0.f);

			drawBitmapText((char*)ob.c_str(), frame_x - 0.05f, frame_y - 0.03f, 0.f);
...
public:
	...
	int getAgentCommand() {
		...
		return agent->best_choice(vis->frame_buffer, vis->recon_ob, vis->next_a0, vis->next_a1, vis->next_a2);
	}
	...
	void update() override {
		if (agent == nullptr) {
			...
			vis->snowy->getCurrentObservation(vis->frame_buffer);
			int a = agent->best_choice(vis->frame_buffer, vis->recon_ob, vis->next_a0, vis->next_a1, vis->next_a2);
			vis->visualize();
			if (game_state == NON_TERMINAL) {
