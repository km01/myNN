...
int main(int argc, char* argv[]) {
	...
	glutInit(&argc, argv);
	snowy::Snowy env(h_w(5, 4), 0.3);
	SnowyLauncher launcher(&env, 30);
	launcher.Run();
	...
	root.append(new Dense(32, 32));
	root.append(new ReLU(32));

	Sequential actor;
	actor.append(new Dense(32, 24));
	actor.append(new ReLU(24));
	actor.append(new Dense(24, 3));
	actor.append(new Softmax(3));

	Sequential critic;
	critic.append(new Dense(32, 24));
	critic.append(new ReLU(24));
	critic.append(new Dense(24, 3));
	...
	Sequential decoder;
	decoder.append(new Dense(state_size, 14 * 3 * 2));
	decoder.append(new LeakyReLU(14 * 3 * 2, 0.1));
	decoder.append(new Conv2DTransposed(c_h_w(14, h_w(3, 2)), h_w(2, 2), c_h_w(12, h_w(4, 3)), h_w(1, 1)));
	decoder.append(new LeakyReLU(12 * 4 * 3, 0.1));
	decoder.append(new Conv2DTransposed(c_h_w(12, h_w(4, 3)), h_w(2, 2), c_h_w(3, h_w(5, 4)), h_w(1, 1)));


	CondGaussianDensityNetwork tr_infer(3);
	tr_infer.append(new Dense(state_size + 3, 128));
	tr_infer.append(new LeakyReLU(128, 0.2));
	tr_infer.append(new Dense(128, 128));
	tr_infer.append(new LeakyReLU(128, 0.2));
	tr_infer.append(new Dense(128,  state_size + state_size));


	AgentModel model(&encoder, &tr_infer, &agent, &decoder, 3, 20);
	model.setCache(1);
	Optimizer optim(model); optim.setLearningRate(0.00025);
