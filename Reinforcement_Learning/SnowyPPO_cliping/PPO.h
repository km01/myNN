...
public:
	...
	Snowy::environment* env;
	Snowy::environment** env_clone;
	...
	void setClipingEPSILON(double _epsilon) {
		EPSILON = _epsilon;
	}
	void allocate(int _n_env, int _sample_sz, int _batch_size) {
		...
		env_clone = new Snowy::environment * [n_env];
		for (int i = 0; i < n_env; i++) {
			env_clone[i] = new Snowy::environment(env->grid_width, env->grid_height);
	...
	void Fit() {
		...
		for (int b = 0; b < n_batch; b++) {
			...
			for (int i = 0; i < batch_size; i++) {
				...
				/* gradient cliping */
				if (memory->batch_target[i] > critic_port[i][0]) {
					if (actor_port[i][memory->batch_a[i]] > (1.0 + EPSILON) * memory->batch_prob[i]) {
						actor_grad[i][memory->batch_a[i]] = -(1.0 + EPSILON) / actor_port[i][memory->batch_a[i]];
					}
					else {
						actor_grad[i][memory->batch_a[i]] = -(memory->batch_target[i] - critic_port[i][0]) / memory->batch_prob[i];
					}
				}
				else {
					if (actor_port[i][memory->batch_a[i]] < (1.0 - EPSILON) * memory->batch_prob[i]) {
				...
				/*					*/
				Policy_loss += actor_grad[i][memory->batch_a[i]] * actor_grad[i][memory->batch_a[i]];
	...
	void MonteCarloSampling(Snowy::environment* env) {
		double R = 0.0;
		int head_point = 0;
		if (env->T == TERMINAL) {
			env->InitSession();
