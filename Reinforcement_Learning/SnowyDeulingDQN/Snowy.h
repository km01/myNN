...
namespace Snowy {
	...
	namespace REWARD {
		const double _die_ = -0.9;
		const double _good_ = 0.2;
		const double _bad_ = -0.3;
	...
	public:
		...
		void SupposeVirtualSituation(double**& virtual_S, double*& virtual_R, bool*& virtual_T) {
			...
			if (man_x > 0) {
				PutAgentAndScoring(man_x - 1, virtual_S[LEFT], virtual_R[LEFT], virtual_T[LEFT]);
			}
			else {
				PutAgentAndScoring(man_x, virtual_S[LEFT], virtual_R[LEFT], virtual_T[LEFT]);
			}

			/*  R I G H T  */
			GetFutureBackGround(virtual_S[RIGHT]);
			if (man_x < grid_width - 1) {
				PutAgentAndScoring(man_x + 1, virtual_S[RIGHT], virtual_R[RIGHT], virtual_T[RIGHT]);
			}
			else {
				PutAgentAndScoring(man_x, virtual_S[RIGHT], virtual_R[RIGHT], virtual_T[RIGHT]);
			}
			/*  H O L D  */
			GetFutureBackGround(virtual_S[HOLD]);
			PutAgentAndScoring(man_x, virtual_S[HOLD], virtual_R[HOLD], virtual_T[HOLD]);
		}
	};
};