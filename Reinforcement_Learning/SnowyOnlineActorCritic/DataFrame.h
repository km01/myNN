	...
	void scaling(double min, double max) {
		...
		for (int c = 0; c < n_cols; c++) {
			...
			for (int r = 1; r < n_rows; r++) {
				if (origin_max < data[r][c]) {
					origin_max = data[r][c];
				}
				if (origin_min > data[r][c]) {
					origin_min = data[r][c];
				}
			}
			for (int r = 0; r < n_rows; r++) {
				data[r][c] = (data[r][c] - origin_min) * ((max - min) / (origin_max - origin_min)) + min;
			}
...
public:
	...
	double scaled_max;
	double scaled_min;
	ImgScaler(double _scaled_max, double _scaled_min) {
		scaled_max = _scaled_max;
		scaled_min = _scaled_min;
	}
	...
	void scale(double** target, int n_data, int imglen) {
		for (int i = 0; i < n_data; i++) {
			for (int j = 0; j < imglen; j++) {
				target[i][j] = target[i][j] * ((scaled_max - scaled_min) / 255.0) + scaled_min;
			}
		}
	}

	void descale(double** target, int n_data, int imglen) {
		for (int i = 0; i < n_data; i++) {
			for (int j = 0; j < imglen; j++) {
				target[i][j] = (target[i][j] - scaled_min) * (255.0 / (scaled_max - scaled_min));
			}
		}
	}
};
...
public:
	...
	void shuffle_order() {
		km::shuffle(order_list, set_size);
	}
