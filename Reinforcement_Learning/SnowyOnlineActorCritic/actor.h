...
public:
	...
	int NoGradDeterministicPolicy(double* const& state) {
		_charge_(state);
		_calculate_(policy_prob);
		return km::argmax(policy_prob, output_size);
	}
	
	int NoGradStochasticPolicy(double* const& state) {
		_charge_(state);
		_calculate_(policy_prob);
		return km::pick(policy_prob, output_size);
	}

	int WithGradDeterministicPolicy(double* const& state) {
		km_1d::copy(in_port[0], state, input_size);
		forward(batch_policy_prob);
		return km::argmax(batch_policy_prob[0], output_size);
	}
	
	int WithGradStochasticPolicy(double* const& state) {
		km_1d::copy(in_port[0], state, input_size);
		forward(batch_policy_prob);
