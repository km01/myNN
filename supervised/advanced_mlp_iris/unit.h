...
#include "mynn_normalizer.h"
enum UNIT {PERCEPTRONS, BN_PERCEPTRONS};
class unit {
public:
	...
	virtual void single_forward_prop(double* to_next) = 0;
	virtual void single_leak_back_prop(double* from_next) = 0;
...
public:
	...
	virtual void save_unit(ofstream& fout) {
		fout << unit_type << endl;
	...
	virtual void single_forward_prop(double* to_next) {
		single_input_X_weight(single_pre_activated_container);
		activator->function(to_next, single_pre_activated_container);
	}

	void single_input_X_weight(double* container) { /*pre_activated_container <- matmul(input, weight)*/
		if (use_bias == true) {
			km_1d::copy(container, bias, output_size);
		}
		else {
			km_1d::fill_zero(container, output_size);
		}
		for (int w = 0; w < output_size; w++) {
			for (int h = 0; h < input_size; h++) {
				container[w] += weight[w*input_size + h] * single_input_container[h];
			}
		}
	}
	virtual void single_leak_back_prop(double* from_next) {
	...
	void batch_input_X_weight(double** container) { /*pre_activated_container <- matmul(input, weight)*/
		if (use_bias == true) {
			for (int m = 0; m < batch_size; m++) {
				for (int w = 0; w < output_size; w++) {
	...
	virtual void batch_forward_prop(double** to_next) {
		batch_input_X_weight(batch_pre_activated_container);
...
public:
	...
	double** batch_un_normalized_container;
	double* single_un_normalized_container;
	bn_perceptrons(){}
	bn_perceptrons(int input_sz, int output_sz, ACTIVATION f, bool use_Bias = true) {
		this->create(input_sz, output_sz, f, use_Bias);
		normalizer = new batch_normalizer(output_size, 1);
		single_un_normalized_container = km_1d::alloc(output_size);
	...
	virtual void alloc_batch_storage(const int& new_batch_size) {
		if (batch_memory_allocated) {
			...
			km_2d::free(batch_input_container, batch_size);
			km_2d::free(batch_pre_activated_container, batch_size);
			km_2d::free(batch_un_normalized_container, batch_size);
		}
		...
		batch_un_normalized_container = km_2d::alloc(batch_size, output_size);
		normalizer->alloc_batch_memory(batch_size);
		batch_memory_allocated = true;
	}
	virtual void save_unit(ofstream& fout) {
		fout << unit_type << endl;
	...
	virtual void single_forward_prop(double* to_next) {
		single_input_X_weight(single_un_normalized_container);
		normalizer->single_normalize(single_pre_activated_container, single_un_normalized_container);
		activator->function(to_next, single_pre_activated_container);
	}
	virtual void single_leak_back_prop(double* from_next) {
		activator->function_prime(single_inner_deltaflow, single_pre_activated_container); //inner_df[m][w] <= d(activated)/d(pre_activated)
		...
		normalizer->single_leak_back_prop(single_inner_deltaflow, single_un_normalized_container);
		km_1d::fill_zero(single_port, input_size);
		for (int h = 0; h < input_size; h++) {
			for (int w = 0; w < output_size; w++) {
				single_port[h] += single_inner_deltaflow[w] * weight[w*input_size + h];
	...
	virtual void batch_forward_prop(double** to_next) {
		batch_input_X_weight(batch_un_normalized_container);
		normalizer->batch_normalize(batch_pre_activated_container, batch_un_normalized_container);
		activator->batch_function(to_next, batch_pre_activated_container);
	}
	...
	virtual void batch_back_prop(double** from_next) {
		...
		normalizer->batch_back_prop(batch_inner_deltaflow, batch_un_normalized_container);
		get_gradients();// get d(Loss)/d(Weight)
		recitfying_delta(); //entrance_port[m][w] <= d(Loss)/d(input[m][w])
	}
	...
	virtual void feed_grad_itself(const double& learning_rate) {
		...
		normalizer->feed_grad_itself(learning_rate);
		for (int i = 0; i < weight_len; i++) {
			weight[i] -= gradients_weight[i] * learning_rate;
		}
	}
	~bn_perceptrons() {
		delete normalizer;
		km_1d::free(single_un_normalized_container);
		km_2d::free(batch_un_normalized_container, batch_size);
	}
};