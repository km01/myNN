...
namespace km {
	...
	double univariate_gaussian_likelihood(const double& x, const double& mu, const double& sigma) {
		static double constant = 1.0 / sqrt(2.0 * 3.14159265358979);

		return constant * exp((x - mu) * (mu - x) / (2.0 * sigma * sigma)) / sigma;
	}
	...
	void setZero(double** const& arr, const int& n_data, const int& len) {
	...
	double mse_loss(darr::v& grad, darr::v const& hyp, darr::v const& tar, const int& len) {
		double loss = 0.0;
		for (int i = 0; i < len; i++) {
			grad[i] = 2.0 * (hyp[i] - tar[i]);
			loss += (hyp[i] - tar[i]) * (hyp[i] - tar[i]);
		}
		return loss;
	}

	double univariate_gaussian_pdf(const double& rv, const double& param_mean, const double& param_stddev) {
		return exp(-((rv - param_mean) * (rv - param_mean) / (2.0 * param_stddev * param_stddev))) / (sqrt(2.0 * 3.141592) * param_stddev);
	}
